using System;

namespace UseCase6
{
    internal class Program
    {
        static void Main(string[] args)
        {
            int PART_TIME = 1;
            int FULL_TIME = 2;
            int WAGE_PER_HOUR = 20;
            int NO_OF_WORKING_DAYS = 2;
            int MAX_HRS_IN_MONTH = 10;

            int empHrs = 0, totalHrs = 0, totalWorkingDays = 0;

            while(totalHrs <= MAX_HRS_IN_MONTH && totalWorkingDays < NO_OF_WORKING_DAYS)
            {
                totalWorkingDays++;
                Random random = new Random();
                int number = random.Next(0, 3);
                switch(number)
                {
                    case 0: PART_TIME:
                        empHrs = 4;
                        break;
                    case 1: FULL_TIME:
                        empHrs = 8;
                        break;
                    default:
                        empHrs = 0;
                        break;
                }
                totalHrs += empHrs;
                Console.WriteLine("Working days of employee: " + totalWorkingDays + "Emp Hours: " + empHrs);
            }
            int totalWage = totalHrs * WAGE_PER_HOUR;
            Console.WriteLine("Total WAgeof Employee is: " + totalWage);
        }
    }
}
